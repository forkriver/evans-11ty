...
async function getImage( src, alt = '', ret = 'imgTag' ) {
	...
	let metadata = await Image(
		src,
		{
			widths:    [ 2048, 1024, 960, 600, 150, "auto" ],
			formats:   [ "webp", "jpeg" ],
	...
	case 'og':
		image = getImageSize( metadata, 2048 );
		return `<meta name="og:image" content=${baseURL}${image.url}">`;
		break;
	// URL for a CSS snippet.
	case 'css':
		let slideImg = getImageSize( metadata, 1024 );
		return slideImg.url;
		break;
...
function getUpcomingMovies( movies ) {
	if ( ! movies ) {
		// If there are no upcoming movies, bail out.
		return;
	}
	var html = '';
	var i    = 0;
	for ( const movie of movies ) {
		i++;
		html += `<p id="movie-${i}">`;
		html += `<a href="${movie.data.permalink}">${movie.data.title}</a>`;
		if ( movie.data.rating ) {
			html += ' ' + getRatingString( movie.data.rating );
		}
		html += '<br />';
...
function getSlideshow( movies ) {
    ...
    html += `<h2 class="evans-slide movie-title"><a href="${movie.data.permalink}">${movie.data.title}</a>`
    if ( movie.data.rating ) {
    	html += ' ' + getRatingString( movie.data.rating );
    }
    html += `</h2>`;
    html += '<p class="evans-slide movie-date-range">' + evansDateRange( movie.data.showtime ) + '</p>';
    html += '<div class="evans-slide excerpt">' + getExcerpt( movie ) + '</div><!-- .evans-slide .excerpt -->';
...
function displayMovie( movie ) {
	var   html = '';
	const movieStartDate = evansDateFormat( movie.data.showtime[0] );
	var   movieRating    = '';
	if ( movie.data.rating ) {
		movieRating = getRatingString( movie.data.rating );
	}
	const movieShowDates = evansDateRange( movie.data.showtime );
	const movieExcerpt   = getExcerpt( movie );
	html += `
<details>
	<summary>
		<a href="${movie.url}" title="${movieStartDate}">${ movie.data.title }</a> ${movieRating} <span class="movie-showtime-summary">${movieShowDates}</span>
	</summary>
	${movieExcerpt}
	${movieShowDates}
</details>
...
/**
 * Gets the slideshow CSS for the home page.
 *
...
async function getSlideshowCSS( movies ) {
	...
	css += `
		...
		/* See the pseudo-content trick here: https://inclusive-components.design/cards/ */
		.evans-slide-wrap a::after {
		  content:'';
			position: absolute;
		  left: 0;
		  right: 0;
...
/**
 * If it's just before or just after Xmas, add a note about gifts.
 *
 * @return string Gift note HTML, or empty string.
 */
function getGiftNote() {
	var html = '';
	if (
		// November, December...
		11 <= DateTime.local( { zone: "America/Winnipeg" } ).month ||
		// ...and January.
		2  >  DateTime.local( { zone: "America/Winnipeg" } ).month
	) {
		html = `
<aside id="gift-suggestion">
			<a href="/gift/"><span class="give-a-gift icon"></span> Not sure what to get for someone on your list? It's easy to give the gift of movies!</a>
</aside>	
	`;
	}
	return html;
...
/**
 * Gets the desired width and type of image.
 *
 ...
 */
function getImageSize( imageData, size = 0, type = 'webp' ) {
	if ( imageData[ type ] ) {
		for ( const img of imageData[ type ] ) {
			if ( size == img.width ) {
				return img;
			}
		}
...
/**
 * Gets the movie rating string.
 *
 * @param  mixed  rating   The rating data. String or Object.
 * @param  string tag      The tag to wrap it in.
 * @param  bool   detail   Whether to include the "advisory" part of the string.
 * @param  string advisory The rating advisory text, if separate from the rating data.
 * @return string          The HTML tag, or an empty string.
 */
function getRatingString( rating, tag = 'span', detail = false, advisory = '' ) {
	var ratingString = '';
	if ( ! rating || 'undefined' == typeof rating ) {
		return '';
	}
	if ( 'string' === typeof rating ) {
		ratingString = rating;
	} 
	if ( 'string' === typeof rating.rating ) {
	...
	if ( detail ) {
		// Adds the rating detail, if present.
		if ( 'string' === typeof( rating.detail ) ){
			ratingString += '; ' + rating.detail;
		} else if ( 0 < advisory.length ){
			ratingString += '; ' + advisory;
		}
	}
	if ( 'div' === tag ) {
		return `<div class="movie-rating">Rated ${ratingString}</div><!-- movie-rating -->`;
	}
	return `<span class="movie-rating">(${ratingString })</span><!-- .movie-rating -->`;
}
